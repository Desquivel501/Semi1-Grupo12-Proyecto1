...
export class ArtistController {
  static async createArtist(req: Request, res: Response) {
    ...
    if (!checkKeys(artist, ["name", "avatar", "birthDate"]) || !file) {
  ...
  static getSongsNotInAlbum(req: Request, res: Response) {
    const { id } = req.params;
    if (!id) return res.status(401).json({ message: "Falta el id" });
    const id_artist = parseInt(id);
    ArtistModel.getSongsNotInAlbum({ id: id_artist }, (response: any, ok: Boolean) => {
      res.status(ok ? 200 : 400).json(response);
    });
  }
